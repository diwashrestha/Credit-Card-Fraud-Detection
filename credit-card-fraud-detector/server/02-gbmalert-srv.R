## Render the infobox
output$gbm_transaction_box <- renderInfoBox({
  infoBox(
    "Transaction",
...
output$gbm_amount_box <- renderInfoBox({
...
output$gbm_fraud_box <- renderInfoBox({
  real_data <-
    predict_fraudgb() %>%
    group_by(predict) %>%
    tally(name = "count") %>%
...
output$gbm_genuine_box <- renderInfoBox({
  real_data <-
    predict_fraudgb() %>%
    group_by(predict) %>%
    tally(name = "count") %>%
...
output$gbm_datatable <- renderDataTable({
  data <-
    predict_fraudgb() %>%
    group_by(predict) %>%
    filter(predict == 1)
  ...
  fwrite(data, "./fraud/gbm.csv", row.names = FALSE)
  DT::datatable(data,
    selection = "single",
...
output$gbm_limeplot <- renderPlot({
  #  input$run_model                                 # button input
  validate(
    need(input$gbm_datatable_rows_selected, "Click on a Row from the Table")
  )

  index <- input$gbm_datatable_rows_selected
...
output$gbm_plot <- renderHighchart({
  data <-
    predict_fraudgb() %>%
    group_by(predict) %>%
    filter(predict == 1)
...
output$gbm_barplot <- renderHighchart({
  data <-
    predict_fraudgb() %>%
    group_by(predict) %>%
    filter(predict == 1)
